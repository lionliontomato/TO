...
const SHEET_EDIT_URL = "https://docs.google.com/spreadsheets/d/10TBRxSI86Ghbx3rUc2SnTQe7iVNnyJ6ycjNAOuiz74Q/edit?usp=sharing";
...
function renderSummary() {
  ...
  const totals = headers.map((header, index) => {
    if(index === 0) return { label: "總筆數", value: records.length, accent: "#bcecff" };
    return {
      label: header,
...
function updateResultText(items, keyword) {
  const text = keyword
    ...
    : `已連結 Google 試算表，目前顯示全部 ${records.length} 筆資料。`;
